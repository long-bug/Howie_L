...
先明确，启动uwsgi的绝对路径命令是什么
	...
	8.3   因此 启动 crm项目的 完整绝对路径命令是
			/opt/CRM/uwsgi     --ini   /opt/CRM/uwsgi.ini  
...
(venv_crm) [root@iZ2zecvz55phtrzqieqc03Z tengine-2.3.2]# make && make install 
...
/opt/venv_crm/bin:/opt/python369/bin:/opt/tngx232/sbin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin::/root/bin
...
### nginx的404页面优化![image-20200219193954237](F:\Python学习笔记\图片\image-20200219193954237.png)
...
[root@s25linux conf]# vim /s25python/40x.html
...
实时监测访客日志的信息
返回上级目录，输入tail -f logs/access.log
...
mkdir /s25proxy
cd /s25proxy
[root@s25linux conf]# vim /s25proxy/index.html
<meta charset=utf8>
...
server   182.92.233.194:85;
server  182.92.233.194:86;
}
    server {
        listen       80;
        server_name  localhost;
